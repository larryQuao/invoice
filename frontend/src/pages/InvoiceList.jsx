...
function InvoiceList() {
  ...
  const handleShareWhatsApp = (invoice) => {
    ...
    if (!invoice.customer_phone) {
      alert('Customer does not have a phone number. Please add one to share via WhatsApp.');
      return;
    }
            ...
            <div key={invoice.id} className="card hover:shadow-md transition-shadow">
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 flex gap-2 flex-wrap">
                ...
                <button
                  ...
                  className="text-sm px-4 py-2 bg-green-100 text-green-700 rounded-lg hover:bg-green-200 transition-colors"
                  title={invoice.customer_phone ? 'Share via WhatsApp' : 'Customer phone number required'}
                >
                  💬 WhatsApp
                </button>
                <Link
